...
if (require.main === module) {
  const testLead = {
    businessName:      'Revive Medical Spa, Denver CO',
    city:              'Denver, CO',
    phone:             '(720) 248-7355',
    website:           'https://revivedripmedspa.com/',
    googleRating:      5,
    googleReviews:     9,
    responseTimeHours: null, // null = no reply
  };
  const out = path.join(__dirname, 'audits', 'test-audit-html.pdf');
  generatePDF(testLead, out)
    .then(() => console.log('✅ PDF generated:', out))
    .catch(console.error);
}
